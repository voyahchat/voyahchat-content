...
Большой выпуск по вашим хотелкам: то, что просили давно, то, о чём писали по ходу беты — и даже то, что успело запрыгнуть в последний вагон.

## Рабочий стол и виджеты
- Новый виджет встаёт в свободное место и сам подстраивает размер. Появился размер ⅔ по высоте
- Виджет быстрого доступа: свой размер для каждой кнопки, скрытие заголовка, свободное перемещение кнопок по сетке
- Первоначальная настройка рабочих столов включается одним переключателем — вместе со всеми настройками и пресетом виджетов
- Виджет «заставка» ставится только на пустой экран

## Быстрый доступ — новые плитки
- Подогрев задних сидений, свет при открытии двери, датчик дождя, помощь при спуске (HDC), ESC, круговой обзор, детский контроль, разблокировка зарядного пистолета, звук пешеходов, камера наблюдения за третьим рядом, освещение салона при открытии дверей и т.п.
- Сервисный режим, режим буксировки, открытие при приближении и закрытие при удалении
- Перезагрузка мультимедиа и TBOX
- Исправлены люк, ПТФ, мойка

## Статус-бар
- Настройки индикаторов пересобраны: выбор показателей в одном диалоге, новые иконки в стиле штатных
- Рекуперация показывается цифрой 1–3
- Добавлены уровень батареи, остаток бензина, целевой заряд режима Save и режим фар
- Режим Save умеет сам ставить порог по текущему заряду (auto save в виджете быстрого доступа)

## Наушники для экрана пассажира
- Если к мультимедиа подключены наушники, то звук из приложений на экране пассажира будет передаваться в наушники. Приложение должно быть на весь экран, а не в виджете

## Сценарии
- Новые действия: массаж водителя и пассажира, подогрев задних сидений, звук пешеходов, открытие и закрытие машины при приближении и удалении, разблокировка зарядного пистолета, включение и выключение Wi-Fi
- Сценарий может включать и выключать другие сценарии
- Триггеры на подключение и отключение от Wi-Fi — любая сеть или конкретная по имени
- При запуске приложения можно выбрать экран: водителя или пассажира
- В сценарии можно переключать рабочие столы

## Кнопки руля
- Назначение кнопок переделано: слева кнопка и тип нажатия, справа действие. Power Hold, круговой обзор, камера третьего ряда добавлены в список

## Голосовые команды
- Голосовая команда «круговой обзор»

## Русификация приборки
- Добавлена поддержка прошивки от 31.07 (7.3.1)

## Зимний пакет
- В пакет добавлены подогрев руля, задних сидений, обдув лобового и обогрев заднего стекла
...
- Быстрый доступ — наборная панель плиток: свет, ПТФ, подогрев/вентиляция сидений и руля, климат (питание, AC, авто, рециркуляция, обдув, обдувалка, температура, обогрев стёкол), стёкла/люк/шторка/багажник, затемнение крыши, режимы движения и тяги, рекуперация, яркость, мойка, сервис дворников, лючки бака и зарядки, Sentry, Power Hold, голосовой ассистент, таск-менеджер, split screen, массаж, громкость по каналам, переход на стол водителя/пассажира, звонок контакту, запуск сценария, запуск приложения. Настройки: число колонок (вплоть до 1 в ряд), стиль и цвет иконок, параметры плиток
- Заставка — большие часы, календарь и погода на весь экран
- Громкость — карточка с каналами (медиа, навигация, голос, телефон) в четырёх размерах

## Рабочие столы и лончер
- Настройка числа виджетов в ряд (3/4) отдельно для водителя и пассажира для каждого стола
- Пункт меню иконки по долгому нажатию «Отправить на задний экран пассажира» / «Убрать с заднего экрана» для перекидывания приложений на экран задних пассажиров
- Виджет погоды кликабелен целиком — открывает штатное приложение
- Доработан виджет телефона

## Звук
- Эквалайзер ViperFX — включение из настроек, список приложений, которые сами не умеют работать с эквалайзером (радио и т.п.)
- Уровни громкости по каналам в настройках с live-применением, опция «устанавливать уровень громкости при старте автомобиля»
- Радио: системное радио в выборе приложения

## Сценарии
- Состояние ремней безопасности (водитель, пассажир, три задних) как условие
- Громкость по каналам, включение/выключение экранов водителя и пассажира, рекуперация 1/2/3, затемнение крыши
- Исправлены сценарии по интервалам времени

## Голосовые команды
- Принудительный электрорежим — «режим только электро», «только электро», «режим принудительно электро», «принудительно электро», «режим полностью электро», «полностью электро»
- Помощь при спуске (вкл) — «помощь при спуске», «помощь на спуске», «спуск с горы»
- Помощь при спуске (выкл) — «выключи помощь при спуске», «отключи помощь при спуске»
- Крыша прозрачная — «сделай крышу прозрачной», «прозрачная крыша»
- Крыша тёмная — «затемнить крышу», «тёмная крыша»
- Тост Voice переработан, кнопка голоса больше не запускает музыку

## Обновление автомобиля
- Скачивание застрявшей прошивки мультимедиа и приборной панели

## Прочее
- Кнопки руля звездочка/фото: выбор ближний свет / авто-свет
- Автоочистка папки загрузок и скачанных APK
- Различные фиксы и улучшение стабильности и быстродействия системы
...
## Виджет медиаплеер
- Тап по обложке открывает приложение, которое играет. Для USB и Bluetooth открывается штатный плеер сразу на странице нужного источника
- Обложки для USB-музыки. Если трек не содержит обложки, то будет отображаться картинка «заглушка»
- После сна виджет поднимает тот источник, который играл до засыпания
- Новая настройка «запускать последний источник из виджета медиаплеер» и отдельный выбор того, что заводить кнопкой play с руля
- Пауза при открывании водительской двери теперь уходит в играющий плеер, а не вслепую всей системе, и не повторяется, пока дверь открыта

## Радио
- Последняя станция возвращается после сна. Условие для работоспособности: на рабочем столе должен быть виджет Медиаплеер

## Кнопки на руле
- Исправлено перепрыгивание трека: следующий трек начинался с позиции предыдущего
- Медиакнопки заработали в Autokit и других зеркалящих приложениях
- Появился запуск рабочего стола с руля с выбором номера стола

## Рабочие столы и виджеты
- Настройка количества рабочих столов, раздельно для водителя и пассажира, с описаниями и подсказками
- Новый виджет: часы с погодой и ланчером приложений

## Сценарии
- В триггерах появились время и температура, в условиях — диапазон времени
- Новые действия: «Рабочий стол» и «Проветривание» (окна)
- Радиус GPS-условия теперь от 5 метров

## Климат
- Синхронизация температуры для всех размеров виджета климата на экране водителя; пассажир крутит свою температуру, водитель свою
- Исправлен летний режим

## Прочее
- Выбор приложений, которые перезапускаются при старте системы
- Исправлено переключение аккаунтов водителя на ОД
- Исправлено переключение DNS для Dreamer Rest
...
## Рабочий стол
- Сохранение в облако настроек рабочих столов
- Запрет дублирования одного приложения в карточке приложений
- Ланчер из дока на пассажирском экране открывается на нужном дисплее

## Медиаплеер и радио
- Радио в медиаплеере
- Bluetooth и USB как источники (руль + медиаплеер)
- Новая логика управления кнопками с руля (на кнопки должен отзываться poweramp и другие приложения при определенных настройках)
- Иконки громкости в виджете медиаплеера пассажира
- Сохранение последней радиостанции для С+
- Автостарт последнего источника музыки из виджета медиаплеера

## Телефон и голос
- Виджет телефона
- Голосовая «Найди контакт…», переработана логика звонков

## Сценарии
- Виджет сценариев
- GPS и температура климата в сценариях
- Добавлена логика И

## Подвеска
- Настройка блокировки подвески: блокировка включается при скорости выше 85 км/ч, отключается ниже 65 км/ч

## Аккаунты / профили
- Голосовые команды профилей водителей А, Б и Ц восстанавливают режимы вождения, положения сидений и зеркал (для машин без МА, не на всех прошивках)

## Приборная панель
- Русификация приборки с прошивкой 7.2.0
- Замена звука поворотников: выбор звука машины, громкость, предварительное прослушивание (пока только для free рест)

## Охрана и прочее
- При постановке на охрану завершаются приложения Я.Навигатор / Карты / Yango Maps для того чтобы после просыпания они «поймали» интернет
- Виджет экрана второго ряда сидений (мультимедиа + климат) + русификация шторки

# 2.8.0

Дата выпуска: **29.06.2026**

Можно обновиться по воздуху, норвегам тоже.

## Рабочие столы и виджеты
- По три рабочих стола для экранов водителя и пассажира
- Виджеты: сидений, ароматизатора, климата, пневмоподвески, погоды, медиаплеер, карточка приложения (можно выводить почти любые приложения)

## Голос и сценарии
- Короткие команды люка/шторки, управление в процентах
- Режим охраны через голос и в сценариях (настройка диалога подтверждения в разделе «Голосовой помощник»)
- Режим power hold через голос и в сценариях (настройка диалога подтверждения в разделе «Голосовой помощник»)
- Исправления в сценариях
- Переключение профилей водителей (A/B/C) голосовой командой «Водитель А/Б/Ц» (только для прошивок МИ, всесторонне не тестировалось)

## Приборная панель
- Русификация приборки отдельным переключателем (только для прошивок приборки H97CSA8155-DAILY-20260104134234-USR) — фри рест с прошивками 7.0.3

## Мультимедиа автомобиля
- Фикс предупреждения пешеходов для дорестов
- Добавлены значения DPI 100 и 300 для приложений
...
## Сплит-скрин
- Полностью переписан. При сильном изменении пропорции окон требуется перезапуск приложения в нём (не баг)

## Кнопки руля
- Заводские комбо (звезда+трубка, звезда+фотик) больше не вызывают наши переопределения при долгом удержании
- При отключении корректно восстанавливается заводской функционал

## Мастер настройки
- Иконки навбара применяются сразу после завершения, без ручной смены в настройках

## Светлая тема
- Исправлен фон главного экрана
- Исправлены иконки «закрыть» в подписке/оплате

## Статус-бар
- Иконка рекуперации по умолчанию выключена
